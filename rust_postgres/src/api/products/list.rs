...
pub async fn get_productlist(
    ...
    Path(page): Path<i32>) -> impl IntoResponse {
    
    let totalrecords_count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM products")
    ...
    let users_result: Vec<Products> = sqlx::query_as(r#"SELECT id::int8,category,descriptions,qty::int8,unit,costprice,sellprice,saleprice,productpicture,alertstocks::int8,criticalstocks::int8 FROM products OFFSET $1 LIMIT $2"#)
    .bind(&offset)
    .bind(&per_page)
